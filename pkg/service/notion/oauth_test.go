...
func TestOAuthService_IsWorkspaceAllowed(t *testing.T) {
	t.Run("allows all workspaces when no restrictions configured", func(t *testing.T) {
		config := notion.OAuthConfig{
			ClientID:     "test-client-id",
			ClientSecret: "test-client-secret",
			RedirectURI:  "http://localhost:8080/api/auth/notion/callback",
			// No AllowedWorkspaceIDs configured
		}
		oauthService := notion.NewOAuthService(config)

		// Should allow any workspace
		gt.B(t, oauthService.IsWorkspaceAllowed("any-workspace-id")).True()
		gt.B(t, oauthService.IsWorkspaceAllowed("another-workspace")).True()
	})

	t.Run("restricts to allowed workspaces when configured", func(t *testing.T) {
		config := notion.OAuthConfig{
			ClientID:            "test-client-id",
			ClientSecret:        "test-client-secret",
			RedirectURI:         "http://localhost:8080/api/auth/notion/callback",
			AllowedWorkspaceIDs: []string{"workspace-1", "workspace-2"},
		}
		oauthService := notion.NewOAuthService(config)

		// Should allow configured workspaces
		gt.B(t, oauthService.IsWorkspaceAllowed("workspace-1")).True()
		gt.B(t, oauthService.IsWorkspaceAllowed("workspace-2")).True()

		// Should deny non-configured workspaces
		gt.B(t, oauthService.IsWorkspaceAllowed("workspace-3")).False()
		gt.B(t, oauthService.IsWorkspaceAllowed("unauthorized-workspace")).False()
	})

	t.Run("handles empty allowed workspace list", func(t *testing.T) {
		config := notion.OAuthConfig{
			ClientID:            "test-client-id",
			ClientSecret:        "test-client-secret",
			RedirectURI:         "http://localhost:8080/api/auth/notion/callback",
			AllowedWorkspaceIDs: []string{}, // Explicitly empty
		}
		oauthService := notion.NewOAuthService(config)

		// Should allow any workspace (same as no restriction)
		gt.B(t, oauthService.IsWorkspaceAllowed("any-workspace")).True()
	})
}
